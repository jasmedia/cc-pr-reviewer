...
**Prompt selection.** Claude reads `REVIEW_PROMPT_CLAUDE` which invokes the PR Review Toolkit plugin's six sub-agents by name. Codex and Gemini share `REVIEW_PROMPT_FILE_BASED`, which references the six bundled Markdown files in `cc_pr_reviewer/pr_review_agents/`. The bundled files are an adapted fork of the upstream toolkit's agent prompts (YAML frontmatter and `## When to invoke` sections stripped, plus CLAUDE.md→AGENTS.md broadening and PR-focused scope phrasing). Keep them in sync with `scripts/sync_pr_review_agents.py` (see below).
...
- **Layered prompt construction.** `build_review_prompt(cli=…)` (pure, called from `_launch_claude`) is the single assembly point. It picks `REVIEW_PROMPT_CLAUDE` or `REVIEW_PROMPT_FILE_BASED` for the base, then optional reviewer-extras / existing-comments / post-inline blocks join with `PROMPT_SECTION_SEP`. All `POST_INLINE_*` suffixes apply to every CLI — they're about `gh` CLI usage, not the reviewing agent. Two load-bearing facts the suffix matrix relies on:
...
- **Upstream sync workflow.** `cc_pr_reviewer/pr_review_agents/*.md` is a static fork of the Claude `pr-review-toolkit` plugin's agent files. The plugin reports its version as `unknown`, so drift is detected by content diff. `scripts/sync_pr_review_agents.py` normalises current upstream (strips frontmatter + `## When to invoke`) and diffs it against a committed baseline snapshot at `scripts/upstream_baseline/*.md`; when those diverge, the script prints `UPSTREAM CHANGED` and exits non-zero. After triaging upstream changes into the bundled files, run `--save-baseline` to lock in the new reference. `scripts/upstream_baseline/` is outside the wheel (pyproject's `packages = ["cc_pr_reviewer"]` excludes `scripts/`), so it stays a maintainer-only artifact.
